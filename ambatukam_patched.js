...
// Variables
const rootDomain = "jembut.workers.dev"; // Ganti dengan domain utama kalian
...
// PATCH START - Tambahkan endpoint /dashboard
...
async function getDashboardHTML() {
    ...
    .map((p, idx) => {
      return `
        <tr class="border-b">
          ...
          <td class="px-4 py-2">${p.proxyIP}</td>
          <td class="px-4 py-2">${p.proxyPort}</td>
          <td class="px-4 py-2">${p.country}</td>
          <td class="px-4 py-2">${p.org}</td>
          <td class="px-4 py-2" id="latency-${idx}">-</td>
          <td class="px-4 py-2">
            <button onclick="checkLatency('${p.proxyIP}', '${p.proxyPort}', ${idx})"
              class="bg-blue-500 text-white px-3 py-1 rounded text-sm">Check</button>
          </td>
        </tr>`;
    })
    .join("\n");
  ...
  <head>
    <meta charset="UTF-8">
    <title>Proxy Checker Dashboard</title>
    <script src="https://cdn.tailwindcss.com"></script>
  </head>
  <body class="bg-gray-100 p-4">
    <div class="max-w-7xl mx-auto bg-white shadow p-6 rounded">
      <h1 class="text-2xl font-bold mb-4">Proxy Checker Dashboard</h1>
    ...
    <script>
      async function checkLatency(ip, port, idx) {
        const el = document.getElementById('latency-' + idx);
