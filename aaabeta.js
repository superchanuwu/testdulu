...
const serviceName = "cloudflare"; // Ganti dengan nama workers kalian
const apiKey = "53abc9675a50fc915d42a175c27b390cfbf11"; // Ganti dengan Global API key kalian (https://dash.cloudflare.com/profile/api-tokens)
const apiEmail = "aizen.suke2@gmail.com"; // Ganti dengan email yang kalian gunakan
const accountID = "04631510c42a6977060d05d134c6d815"; // Ganti dengan Account ID kalian (https://dash.cloudflare.com -> Klik domain yang kalian gunakan)
const zoneID = "b67941515a695a3e235e72311922c318"; // Ganti dengan Zone ID kalian (https://dash.cloudflare.com -> Klik domain yang kalian gunakan)
...
const PORTS = [443, 80];
const PROTOCOLS = ["cftroljan", "cfvlcfess", "ss", "cfvmcfess"];
...
const PROXY_HEALTH_CHECK_API = "https://cmeta.my.id/api/v1/check";
const CONVERTER_URL =
...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    // Build URI
    const uri = new URL(`cftroljan://${hostName}`);
    ...
    for (let i = startIndex; i < startIndex + PROXY_PER_PAGE; i++) {
      ...
      for (const port of PORTS) {
        ...
        for (const protocol of PROTOCOLS) {
          ...
          uri.searchParams.set("sni", port == 80 && protocol == "cfvlcfess" ? "" : hostName);
    ...
    return `An error occurred while generating the cfvlcfess configurations. ${error}`;
  }
}
      ...
      if (url.pathname.startsWith("/sub")) {
        ...
        if (apiPath.startsWith("/domains")) {
          ...
          for (const proxy of proxyList) {
            const uri = new URL(`cftroljan://${fillerDomain}`);
            uri.searchParams.set("encryption", "none");
            uri.searchParams.set("type", "ws");
            ...
            for (const port of filterPort) {
              for (const protocol of filterVPN) {
                ...
                uri.searchParams.set("security", port == 443 ? "tls" : "none");
                uri.searchParams.set("sni", port == 80 && protocol == "cfvlcfess" ? "" : APP_DOMAIN);
    ...
    .pipeTo(
      new WritableStream({
        async write(chunk, controller) {
          ...
          if (protocol === "Cftroljan") {
            protocolHeader = parseCftroljanHeader(chunk);
          } else if (protocol === "Cfvlcfess") {
            protocolHeader = parseCfvlcfessHeader(chunk);
          } else if (protocol === "Cfvmcfess") {
            protocolHeader = parseCfvmcfessHeader(chunk);
...
async function protocolSniffer(buffer) {
  if (buffer.byteLength >= 62) {
    const cftroljanDelimiter = new Uint8Array(buffer.slice(56, 60));
    if (cftroljanDelimiter[0] === 0x0d && cftroljanDelimiter[1] === 0x0a) {
      if (cftroljanDelimiter[2] === 0x01 || cftroljanDelimiter[2] === 0x03 || cftroljanDelimiter[2] === 0x7f) {
        if (cftroljanDelimiter[3] === 0x01 || cftroljanDelimiter[3] === 0x03 || cftroljanDelimiter[3] === 0x04) {
          return "Cftroljan";
        }
      }
    }
  ...
  const cfvlcfessDelimiter = new Uint8Array(buffer.slice(1, 17));
...
function parseCfvlcfessHeader(cfvlcfessBuffer) {
  const version = new Uint8Array(cfvlcfessBuffer.slice(0, 1));
  let isUDP = false;

  const optLength = new Uint8Array(cfvlcfessBuffer.slice(17, 18))[0];

  const cmd = new Uint8Array(cfvlcfessBuffer.slice(18 + optLength, 18 + optLength + 1))[0];
  if (cmd === 1) {
  } else if (cmd === 2) {
    isUDP = true;
  } else {
  ...
  const portBuffer = cfvlcfessBuffer.slice(portIndex, portIndex + 2);
  const portRemote = new DataView(portBuffer).getUint16(0);

  let addressIndex = portIndex + 2;
  const addressBuffer = new Uint8Array(cfvlcfessBuffer.slice(addressIndex, addressIndex + 1));
  ...
  switch (addressType) {
    case 1: // For IPv4
      addressLength = 4;
      addressValue = new Uint8Array(cfvlcfessBuffer.slice(addressValueIndex, addressValueIndex + addressLength)).join(".");
      break;
    case 2: // For Domain
      addressLength = new Uint8Array(cfvlcfessBuffer.slice(addressValueIndex, addressValueIndex + 1))[0];
      addressValueIndex += 1;
      addressValue = new TextDecoder().decode(cfvlcfessBuffer.slice(addressValueIndex, addressValueIndex + addressLength));
      break;
    case 3: // For IPv6
      addressLength = 16;
      const dataView = new DataView(cfvlcfessBuffer.slice(addressValueIndex, addressValueIndex + addressLength));
      const ipv6 = [];
      for (let i = 0; i < 8; i++) {
        ipv6.push(dataView.getUint16(i * 2).toString(16));
      }
  ...
  return {
    ...
    portRemote: portRemote,
    rawDataIndex: addressValueIndex + addressLength,
    rawClientData: cfvlcfessBuffer.slice(addressValueIndex + addressLength),
    version: new Uint8Array([version[0], 0]),
    isUDP: isUDP,
  };
}

function parseCftroljanHeader(buffer) {
  ...
  buildProxyGroup() {
    ...
    for (let i = 0; i < this.proxies.length; i++) {
      ...
      for (let x = 0; x < proxyData.list.length; x++) {
        const indexName = ["cftroljan TLS", "cfvlcfess TLS", "SS TLS", "cftroljan NTLS", "cfvlcfess NTLS", "SS NTLS"];
