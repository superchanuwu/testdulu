...
function md5Concat(...arrays) {
  const combined = arrays.reduce((acc, val) => acc.concat(Array.from(val)), []);
  const wordArray = CryptoJS.lib.WordArray.create(Uint8Array.from(combined));
...
async function kdf(keyBytes, pathArray) {
  let currentKey = await hmacSha256(new TextEncoder().encode("VMess AEAD KDF"), keyBytes);
  for (const path of pathArray) {
    currentKey = await hmacSha256(path, currentKey);
  }
  return currentKey;
}
...
function createBinaryReader(stream) {
...
async function decryptVmessAEAD(reader, uuid) {
  const authId = await reader.readBytes(16);
  const len = await reader.readBytes(18);
  const nonce = await reader.readBytes(8);

  const salt = "c48619fe-8f02-49e0-b9e9-edf763e17e21";
  const key = md5Concat(new TextEncoder().encode(uuid), new TextEncoder().encode(salt));

  const headerLenKey = (await kdf(key, [
    new TextEncoder().encode("VMess Header AEAD Key_Length"),
    authId,
    nonce,
  ])).slice(0, 16);
  const headerLenIv = (await kdf(key, [
    new TextEncoder().encode("VMess Header AEAD Nonce_Length"),
    authId,
    nonce,
  ])).slice(0, 12);
  ...
  const cmd = await reader.readBytes(headerLength + 16);

  const payloadKey = (await kdf(key, [
    new TextEncoder().encode("VMess Header AEAD Key"),
    authId,
    nonce,
  ])).slice(0, 16);
  const payloadIv = (await kdf(key, [
    new TextEncoder().encode("VMess Header AEAD Nonce"),
    authId,
    nonce,
  ])).slice(0, 12);

  const decryptedPayload = await decryptAESGCM(cmd, payloadKey, payloadIv, authId);
...
const serviceName = "cuki"; // Ganti dengan nama workers kalian
