...
// [VMess AEAD] Fungsi md5Concat - hash UUID + salt
...
// [VMess AEAD] Fungsi kdf - key derivation function
...
// [VMess AEAD] Fungsi utama decryptVmessAEAD - mulai proses dekripsi
...
// [VMess AEAD] Dekripsi panjang header (len) dengan AES-GCM
      .filter(Boolean);
  }
...
// [VMess AEAD] Dekripsi payload header (cmd) dengan AES-GCM
...
async function protocolSniffer(reader, earlyData, uuid) {
  const peek = await reader.peek(16);
  if (!peek || peek.length < 16) return null;

  try {
// [VMess AEAD] Pemanggilan decryptVmessAEAD di dalam protocolSniffer()
    const payload = await decryptVmessAEAD(reader, uuid);
    return parseSselvHeader(payload);
  }
  } catch (err) {
    console.log("Not VMess AEAD:", err);
  }
